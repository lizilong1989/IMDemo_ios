...
- (void)isIgnoreGroup:(BOOL)isIgnore
{
    ...
    [[EMClient sharedClient].groupManager asyncIgnoreGroupPush:_group.groupId ignore:isIgnore success:^{
        [weakSelf hideHud];
...
- (void)saveAction:(id)sender
{
    if (_blockSwitch.isOn != _group.isBlocked) {
        ...
        [self showHudInView:self.view hint:NSLocalizedString(@"group.setting.save", @"set properties")];
        if (_blockSwitch.isOn) {
            [[EMClient sharedClient].groupManager asyncBlockGroup:_group.groupId success:^(EMGroup *aGroup) {
                [weakSelf hideHud];
                [weakSelf showHint:NSLocalizedString(@"group.setting.success", @"set success")];
            } failure:^(EMError *aError) {
                [weakSelf hideHud];
            ...
            [[EMClient sharedClient].groupManager asyncUnblockGroup:_group.groupId success:^(EMGroup *aGroup) {
                [weakSelf hideHud];
                [weakSelf showHint:NSLocalizedString(@"group.setting.success", @"set success")];
            } failure:^(EMError *aError) {
                [weakSelf hideHud];
